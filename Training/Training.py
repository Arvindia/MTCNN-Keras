...
'''Last updated on 2020.04.07 09:53'''
...
def main(args):
    ...
    lengths = np.array([len(neg_info) / 3, len(pos_info), len(part_info), len(ldmk_info)])
    # Get the dataset index with the smallest amount rate
    min_index = lengths.argsort()[0]
    if min_index == 0:
        smallest_set = neg_info
        branch_batch_size = int(BATCH_SIZE / 7 * 3)
    elif min_index == 1:
        smallest_set = pos_info
        branch_batch_size = int(BATCH_SIZE / 7)
    elif min_index == 2: 
        smallest_set = part_info
        branch_batch_size = int(BATCH_SIZE / 7)
    elif min_index == 3:
        smallest_set = ldmk_info
        branch_batch_size = int(BATCH_SIZE / 7 * 2)
    else:
    ...
    for i in range(EPOCHS):
        ...
        for j in range(num_of_iters):
            ...
            neg_batch = neg_info[int(j * (BATCH_SIZE / 7 * 3)): int((j + 1) * (BATCH_SIZE / 7 * 3))]
            pos_batch = pos_info[int(j * (BATCH_SIZE / 7)): int((j + 1) * (BATCH_SIZE / 7))]
            part_batch = part_info[int(j * (BATCH_SIZE / 7)): int((j + 1) * (BATCH_SIZE / 7))]
            ldmk_batch = ldmk_info[int(j * (BATCH_SIZE / 7 * 2)): int((j + 1) * (BATCH_SIZE / 7 * 2))]
            ...
            patience_count = 0 
            model.compile(optimizer = SGD(lr = lr), loss = custom_loss)
            ...
            if j % 200 == 0:
                model_name = MODEL + '_train_epoch_' + str(i) + '_iter_' + str(j) + '.h5'
